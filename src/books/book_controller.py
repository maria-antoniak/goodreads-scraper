...
def scrape_book(book_id_title):
    ...
    return {
        ...
        "numeric_book_id": get_book_service.get_numeric_id()
        if config_numeric_book_id is True
        else None,
        "book_title": get_book_service.get_title()
        if config_book_title is True
        else None,
        "book_series": get_book_service.get_book_series()
        if config_book_series is True
        else None,
        "book_series_uri": get_book_service.get_series_uri()
        ...
        "num_ratings": get_book_service.get_number_of_ratings()
        if config_num_ratings is True
        else None,
        "num_reviews": get_book_service.get_number_of_reviews()
        if config_num_reviews is True
        else None,
        "average_rating": get_book_service.get_average_rating()
        if config_average_rating is True
        else None,
        "rating_distribution": get_book_service.get_rating_distribution()
