...
from src.get_book_ids.writer.writer import _is_directory_created, write_to_file
...
class TestWriter:
    ...
    def test_ut_write_file_should_write_book_id_to_empty_file(self):
        open_mock = mock_open()
        with patch("src.get_book_ids.writer.writer.codecs.open", open_mock, create=True):
